...
def compute_chr_score(row):
    ...
    combined_text = str(row.get('Nom du destinataire', '')) + ' ' + str(row.get('Adresse destinataire', ''))
...
if uploaded_file:
    ...
    st.dataframe(data[['Nom du destinataire', 'Adresse destinataire', 'ville destinataire', 'score_CHR', 'identification_CHR']])
